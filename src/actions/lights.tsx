...
import { apiUrl, username } from './../philipsUrl';
...
export const updateLight = (isOn: boolean, id: string, brightnessValue: number): AppActions => ({
    type: UPDATE_LIGHT,
    isOn,
    id,
    brightnessValue
...
export const startFetchLights = () => {
    return (dispatch: Dispatch<AppActions>, getState: () => AppState) => {
        ...
        const url = `${apiUrl}/api/${username}/lights`;

        axios.get(url)
            .then((response: any) => {
                return dispatch(
                    lightsFetched(response.data)
                );
            })
            .catch((error: any) => {
...
export const startUpdateLight = (id: string, isOn: boolean, brightnessValue: number) => {
    return (dispatch: Dispatch<AppActions>, getState: () => AppState) => {

        const url = `${apiUrl}/api/${username}/lights/${id}/state`;
